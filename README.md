...
## Quantum-Holographic Amplitude Engine (`qh/`)

A SWI-Prolog library implementing complex-amplitude quantum states integrated
with the [PITA](https://github.com/friguzzi/cplint) probabilistic reasoning pack.

```
qh/
├── quantum_holographic.pl  # Core amplitude engine + DSL operators
├── qh_cplint.pl            # PITA/cplint bridge (qh_measure expansion)
├── qh_bell.pl              # Bell-state correlations example
└── qh_examples.pl          # Interference, unitary mix, negativity, Born rule
```
...
|-------|-------|----------|
| destructive interference | 0.0 | 0.0 |
| after mix dark/bright | 0.5 / 0.5 | 0.5 / 0.5 |
| quasi-negativity | 0.25 | 0.25 |
| Born dark/bright | 0.36 / 0.64 | 0.36 / 0.64 |
| P(measure dark/bright) | 0.36 / 0.64 | 0.36 / 0.64 |
| P(readout bright) | 0.612 | 0.612 |
| Bell joint up,up / down,down / up,down | 0.64 / 0.36 / 0.0 | matches \|amp\|² |
| P(left up) / P(same spin) | 0.64 / 1.0 | ok |
| P(noise ok) | 0.99 | 0.99 |
| P(reported up \| left up) | 0.95 | 0.95 |

Only remaining noise: three cosmetic bddem export warnings from the cplint pack itself.

### Key Implementation Note

`qh_measure` is intercepted via `user:term_expansion/2` so expanded clauses
load through the normal file path. The old `assertz` path was wiped by PITA's
`end_of_file` zero-clause re-emit.

### Run

```bash
# Requires: SWI-Prolog 8+ with pack(pita) installed
# swipl -g "pack_install(pita)" -t halt
